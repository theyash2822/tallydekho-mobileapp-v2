...
const newReturnItem = (): ReturnItem => ({
  ...
  quantity: '1', unit: 'Pcs', warehouseId: '',
  unitPrice: '', discount: '0', taxRate: 18,
});
...
export default function CreateCreditNoteScreen() {
  ...
  const [noteNo, setNoteNo] = useState('CN-TD-00001');
  ...
  const creditTotal = subtotal + totalTax;
  ...
  const handleSubmit = async () => {
    ...
    try {
      await createApi.createCreditNote({
        companyGuid: company?.guid,
        isOptional, noteNo, date, customerName, refInvoice, narration,
        items: returnItems.map((item, i) => ({ ...item, ...totals[i] })),
        creditTotal,
      });
      Toast.show({ type: 'success', text1: 'Credit note created!', text2: noteNo });
      navigation.goBack();
    } catch (e: any) { setError(e?.message ?? 'Failed to create credit note'); }
    ...
    <SafeAreaView style={s.safe}>
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
        <View style={s.header}>
          ...
          <AppText style={s.headerTitle}>Create Credit Note</AppText>
          <View style={s.optRow}>
        ...
        <ScrollView contentContainerStyle={s.content} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
          ...
          <View style={s.row2}>
            <View style={s.flex1}><F label="Credit Note No." value={noteNo} onChange={setNoteNo} placeholder="CN-TD-00001" /></View>
            <View style={s.flex1}><F label="Date" value={date} onChange={setDate} placeholder="YYYY-MM-DD" /></View>
          </View>
          ...
          {/* Return Items */}
          <View style={s.sH}><AppText style={s.sHText}>Return Items</AppText></View>
          {returnItems.map((item, idx) => (
            <Card key={item.id} style={s.itemCard}>
              ...
              <TextInput style={s.input} value={item.productName} onChangeText={v => updateItem(item.id, 'productName', v)}
              ...
              <View style={s.row2}>
                <View style={{ width: 70 }}>
                  ...
                  <TextInput style={[s.input, s.smallInput]} value={item.quantity} onChangeText={v => updateItem(item.id, 'quantity', v)}
                ...
                <View style={{ flex: 1 }}>
                  ...
                  <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ gap: 4 }}>
                    {STATIC_UNITS.slice(0, 5).map(u => (
                      <TouchableOpacity key={u} onPress={() => updateItem(item.id, 'unit', u)}
                ...
                <View style={{ width: 80 }}>
                  ...
                  <TextInput style={[s.input, s.smallInput]} value={item.unitPrice} onChangeText={v => updateItem(item.id, 'unitPrice', v)}
          ...
          <TouchableOpacity onPress={() => setReturnItems(p => [...p, newReturnItem()])} style={s.addRow}>
          ...
          <Card style={s.summaryCard}>
            ...
            <View style={s.grandRow}>
              <AppText style={s.grandLabel}>Credit Total</AppText>
              <AppText style={s.grandValue}>₹{creditTotal.toFixed(2)}</AppText>
            </View>
          </Card>
        ...
        <View style={s.footer}>
          <Button label={loading ? 'Submitting...' : 'Submit Credit Note'} onPress={handleSubmit} loading={loading} />
        </View>
...
function F({ label, value, onChange, placeholder, multiline }: any) {
  return (
    <View style={{ marginBottom: Spacing.sm }}>
      ...
      <TextInput style={[s.input, multiline && { height: 72, textAlignVertical: 'top' }]}
        ...
        placeholderTextColor={Colors.textTertiary} multiline={multiline} allowFontScaling={false} />
    </View>
  );
}
...
const s = StyleSheet.create({
  ...
  input: { borderWidth: 1, borderColor: Colors.borderDefault, borderRadius: Radius.sm, backgroundColor: Colors.cardBg, paddingHorizontal: Spacing.md, paddingVertical: 10, fontSize: Typography.base, color: Colors.textPrimary, marginBottom: Spacing.xs },
  ...
  itemHead: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: Spacing.xs },
  ...
  itemFoot: { flexDirection: 'row', justifyContent: 'flex-end', borderTopWidth: 1, borderTopColor: Colors.borderDefault, paddingTop: Spacing.xs, gap: 6 },
