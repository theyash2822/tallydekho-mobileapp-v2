...
export default function CreateSalesOrderScreen() {
  ...
  const [orderNo, setOrderNo] = useState('SO-TD-00001');
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await createApi.createSalesOrder({
        companyGuid: company?.guid,
        isOptional, orderNo, orderDate, customerName,
        paymentTerm, dueDate, validityPeriod, warehouseId, narration,
        items: lineItems.map((item, i) => ({ ...item, ...calcs[i] })),
        logistics,
        grandTotal,
      });
      const orderId = res.data?.id ?? orderNo;
      setSubmittedOrderId(orderId);
      setSubmitted(true);
      Toast.show({ type: 'success', text1: 'Sales order created!', text2: orderNo });
    } catch (e: any) {
  ...
  const handleConvertToInvoice = () => {
    navigation.navigate('CreateSalesInvoice', { prefillData: { voucherNumber: orderNo } });
  };
    ...
    <SafeAreaView style={s.safe}>
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
        <View style={s.header}>
          ...
          <AppText style={s.headerTitle}>Create Sales Order</AppText>
          <View style={s.optRow}>
        ...
        >
          ...
          {submitted && submittedOrderId && (
            <Card style={s.successCard}>
              <AppText style={s.successTitle}>✓ Sales Order Created!</AppText>
              <AppText style={s.successSub}>{orderNo}</AppText>
              <TouchableOpacity onPress={handleConvertToInvoice} style={s.convertBtn}>
                <AppText style={s.convertBtnText}>Convert to Sales Invoice →</AppText>
              </TouchableOpacity>
            </Card>
          )}
          ...
          <View style={s.row2}>
            <View style={s.flex1}>
              <F label="Order Number" value={orderNo} onChange={setOrderNo} placeholder="SO-TD-00001" />
            </View>
            <View style={s.flex1}>
              <F label="Order Date" value={orderDate} onChange={setOrderDate} placeholder="YYYY-MM-DD" />
            </View>
          </View>
          ...
          <View style={s.row2}>
            ...
            <View style={s.flex1}>
              <F label="Validity (Days)" value={validityPeriod} onChange={setValidityPeriod} placeholder="30" />
            </View>
          </View>
        ...
        <View style={s.footer}>
          {!submitted ? (
            <Button label={loading ? 'Submitting...' : 'Submit Sales Order'} onPress={handleSubmit} loading={loading} />
          ) : (
            <Button label="Convert to Sales Invoice" onPress={handleConvertToInvoice} />
          )}
        </View>
