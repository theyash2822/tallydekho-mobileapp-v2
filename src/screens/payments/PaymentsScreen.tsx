...
interface PaymentEntry { id: string; date: string; partyName: string; amount: number; mode: string; voucherNumber: string; }
...
export default function PaymentsScreen() {
  ...
  const fetch = useCallback(async () => {
    ...
    try {
      ...
      setItems(data);
      setTotal(data.reduce((s: number, i: PaymentEntry) => s + i.amount, 0));
  ...
  return (
    <SafeAreaView style={s.safe}>
      <View style={s.header}>
        ...
        <AppText style={s.title}>Payments</AppText>
        <View style={{ width: 44 }} />
      </View>
      {error && <ErrorBanner message={error} onRetry={fetch} />}
      <Card style={s.totalCard}><AppText style={s.totalLabel}>Total Payments</AppText><AppText style={s.totalValue}>{fmt(total)}</AppText></Card>
      <FlatList
        ...
        ListEmptyComponent={!loading ? <View style={s.empty}><AppText style={s.emptyText}>{error ? '' : 'No payments found'}</AppText></View> : null}
      />
    </SafeAreaView>
  );
}
